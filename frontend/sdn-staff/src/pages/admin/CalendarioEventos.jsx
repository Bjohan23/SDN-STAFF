...
const CalendarioEventos = () => {
  ...
  const handleEventMove = async ({ event, start, end }) => {
    try {
      ...
      await axios.put(`/api/eventos/${event.id}`, eventoActualizado);
      
      // Actualizar en el estado local
      setEventos(prev => prev.map(e => 
        e.id === event.id 
          ? { ...e, fecha_inicio: start.toISOString(), fecha_fin: end.toISOString() }
          : e
      ));
  ...
  const handleCreateEvent = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('/api/eventos', nuevoEvento);
  ...
  const handleEditEvent = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.put(`/api/eventos/${selectedEvent.id}`, selectedEvent.resource);
      setEventos(prev => prev.map(e => 
        e.id === selectedEvent.id ? response.data.data : e
      ));
  ...
  const resolverConflicto = async (conflicto, accion) => {
    try {
      if (accion === 'mover_segundo') {
        ...
        const duracion = new Date(conflicto.evento2.fecha_fin) - new Date(conflicto.evento2.fecha_inicio);
        const nuevoFin = new Date(nuevoInicio.getTime() + duracion);
        
        await axios.put(`/api/eventos/${conflicto.evento2.id}`, {
          ...conflicto.evento2,
        ...
        await axios.put(`/api/eventos/${conflicto.evento1.id}`, {
          ...conflicto.evento1,
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setNuevoEvento(prev => ({ ...prev, nombre: e.target.value }))}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setNuevoEvento(prev => ({ ...prev, fecha_inicio: e.target.value }))}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        required
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setNuevoEvento(prev => ({ ...prev, fecha_fin: e.target.value }))}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        required
                      />
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => setNuevoEvento(prev => ({ ...prev, categoria: e.target.value }))}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                      >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setNuevoEvento(prev => ({ ...prev, capacidad_maxima: e.target.value }))}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                      min="1"
                    />
                  </div>
                  ...
                  <div className="flex items-center space-x-4">
                    <label className="flex items-center">
                      <input
                        ...
                        onChange={(e) => setNuevoEvento(prev => ({ ...prev, requiere_registro: e.target.checked }))}
                    ...
                    <label className="flex items-center">
                      <input
                        ...
                        onChange={(e) => setNuevoEvento(prev => ({ ...prev, es_publico: e.target.checked }))}
                        className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                      />
                      <span className="ml-2 text-sm text-gray-700">Es público</span>
                    </label>
                  </div>
                </div>
                    ...
                    <div className="flex items-start justify-between">
                      <div>
                        <h4 className="font-medium text-red-800">
                          {conflicto.evento1.nombre} y {conflicto.evento2.nombre}
                        </h4>
                        <p className="text-sm text-red-600 mt-1">
                          Se superponen desde {conflicto.inicio.toLocaleString()} 
                      ...
                      <div className="flex space-x-2">
                        <button
                          onClick={() => resolverConflicto(conflicto, 'mover_segundo')}
                          className="px-3 py-1 bg-blue-600 text-white text-sm rounded hover:bg-blue-700"
                        >
                          Mover segundo
                        </button>
                        <button
                          onClick={() => resolverConflicto(conflicto, 'reducir_duracion')}

...
class CredencialService {
  ...
  static async crearCredencial(datosCredencial, userId = null) {
    try {
      ...
      const tipoCredencial = await TipoCredencial.findByPk(datosCredencial.id_tipo_credencial);
      ...
      const codigoUnico = await this.generarCodigoUnico(datosCredencial.id_evento);
      ...
      // Generar QR
      const qrData = QRGeneratorService.generateSecureQRData(credencial);
      
      // Actualizar credencial con datos del QR
      ...
      await this.crearAccesosPorDefecto(credencial.id_credencial, tipoCredencial);
   ...
   */
  static async crearAccesosPorDefecto(idCredencial, tipoCredencial) {
    const accesosPorDefecto = this.getAccesosPorTipo(tipoCredencial.nombre_tipo);
    
    for (const acceso of accesosPorDefecto) {
      await AccesoEvento.create({
        id_credencial: idCredencial,
        id_evento: tipoCredencial.id_evento,
        ...acceso
      });
    }
  }
  ...
  static getAccesosPorTipo(tipoNombre) {
    const accesos = {
      'visitante': [
        { tipo_acceso: 'entrada_general', descripcion_acceso: 'Acceso general al evento' }
      ],
      'expositor': [
        { tipo_acceso: 'entrada_general', descripcion_acceso: 'Acceso general al evento' },
        { tipo_acceso: 'area_expositores', descripcion_acceso: 'Acceso al área de expositores' },
        { tipo_acceso: 'zona_catering', descripcion_acceso: 'Acceso a zona de catering' }
      ],
      'personal': [
        { tipo_acceso: 'entrada_general', descripcion_acceso: 'Acceso general al evento' },
        { tipo_acceso: 'area_staff', descripcion_acceso: 'Acceso al área de personal' },
        { tipo_acceso: 'backstage', descripcion_acceso: 'Acceso a backstage' }
      ],
      'prensa': [
        { tipo_acceso: 'entrada_general', descripcion_acceso: 'Acceso general al evento' },
        { tipo_acceso: 'area_prensa', descripcion_acceso: 'Acceso al área de prensa' },
        { tipo_acceso: 'sala_conferencias', descripcion_acceso: 'Acceso a sala de conferencias' }
      ],
      'vip': [
        { tipo_acceso: 'entrada_general', descripcion_acceso: 'Acceso general al evento' },
        { tipo_acceso: 'zona_vip', descripcion_acceso: 'Acceso a zona VIP' },
        { tipo_acceso: 'area_networking', descripcion_acceso: 'Acceso a área de networking' },
        { tipo_acceso: 'zona_catering', descripcion_acceso: 'Acceso a zona de catering premium' },
  ...
  static async obtenerCredencialCompleta(idCredencial) {
    return await Credencial.findByPk(idCredencial, {
      include: [
        {
  ...
  static async generarCredencialFisica(idCredencial) {
    try {
      ...
      if (!plantilla) {
        plantilla = await this.obtenerPlantillaPorDefecto(credencial.id_tipo_credencial);
      }
      ...
      // Generar PDF
      const pdf = await PDFGeneratorService.generateCredentialPDF(credencial, plantilla, qrImage);
      
      // Guardar archivos si es necesario
      const archivos = await this.guardarArchivosCredencial(credencial, qrImage, pdf);
  ...
  static async guardarArchivosCredencial(credencial, qrImage, pdf) {
    try {
      const uploadsDir = path.join(process.cwd(), 'uploads', 'credenciales', credencial.id_evento.toString());
  ...
  static async generarCredencialesLote(datosCredenciales, userId = null) {
    ...
    for (let i = 0; i < datosCredenciales.length; i++) {
      try {
        const credencial = await this.crearCredencial(datosCredenciales[i], userId);
  ...
  static async obtenerEstadisticas(idEvento = null) {
    ...
    const porTipo = await Credencial.findAll({
      ...
      group: ['tipoCredencial.id_tipo_credencial', 'tipoCredencial.nombre_tipo'],
      raw: true
    });
